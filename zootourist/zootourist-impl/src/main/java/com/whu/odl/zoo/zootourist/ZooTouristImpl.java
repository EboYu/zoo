...
public class ZooTouristImpl implements ZooTouristService, DataTreeChangeListener<ZooFoods> {
    ...
    public Future<RpcResult<Void>> addTourist(AddTouristInput input) {
        ...
        if (!checkNumOfTourists()) {
            return Futures.immediateFailedFuture(new Exception("System Error: too much tourists or too few animals"));
        } else {
    ...
    private void checkTouristID(int touristID) {
        if (touristID <= 999) {
            ...
            LOG.error("Illegal touristID " + touristID);
        }
    }

    private boolean checkNumOfTourists() {
